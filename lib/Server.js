...
var packageToolkit = require("package-webpack-server-toolkit");
...
function Server(compiler, options) {
    ...
    app.use(packageToolkit.packageDevParser(options))
        .use(packageToolkit.mockParser(options))
        .use(packageToolkit.velocityParser(options));
    ...
    app.get("/__qwebpack_server__/live.bundle.js", function(req, res) {
        res.setHeader("Content-Type", "application/javascript");
        liveJs.pipe(res);
    });

    app.get("/qwebpack-server.js", function(req, res) {
        res.setHeader("Content-Type", "application/javascript");
        inlinedJs.pipe(res);
    });

    app.get("/qwebpack-server/*", function(req, res) {
    ...
    app.get("/qwebpack-server", function(req, res) {
        ...
        var path = this.middleware && this.middleware.getFilenameFromUrl(options.publicPath || "/") || '/';
            ...
            content.forEach(function(item) {
                ...
                if (fs.statSync(p).isFile()) {
                    ...
                    if (/\.js$/.test(item)) {
                        ...
                        res.write(item);
                        res.write(') (<a href="');
                        res.write(baseUrl.replace(/(^(https?:\/\/[^\/]+)?\/)/, "$1qwebpack-server/") + htmlItem);
                        res.write('">qwebpack-server</a>)</li>');
                    }
                } else {
                    res.write('<li>');
                    res.write(item);
